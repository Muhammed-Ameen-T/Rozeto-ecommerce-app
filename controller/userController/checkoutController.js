...
export const validateCoupon = async (req, res) => {
    try {
        ...
        if (!userId) {
            return res.status(HttpResCode.BAD_REQUEST).json({ isValid: false, errorMessage: 'User ID is missing' });
        }

        if (!cartTotal || cartTotal <= 0) {
            return res.status(HttpResCode.BAD_REQUEST).json({ isValid: false, errorMessage: 'Invalid cart total' });
        }
...
export const removeCoupon = async (req, res) => {
    try {
        ...
        if (!userId) {
            return res.status(HttpResCode.BAD_REQUEST).json({ success: false, errorMessage: 'User ID is missing' });
        }

        // Ensure the coupon details exist in the session
        if (!req.session.coupon || !req.session.coupon.code) {
            return res.status(HttpResCode.BAD_REQUEST).json({ success: false, errorMessage: 'No coupon applied' });
        }
        ...
        if (result.nModified === 0) {
            return res.status(HttpResCode.BAD_REQUEST).json({ success: false, errorMessage: 'Coupon not found or user ID not in usedBy array' });
        }
...
export const placeOrder = async (req, res) => {
    try {
        ...
        const userId = req.session.user;
        if (!userId) {
            return res.status(HttpResCode.BAD_REQUEST).json({ error: 'User not logged in' });
        }

        // Check address validity
        const address = await Address.findById(selectedAddressId);
        if (!address) {
            return res.status(HttpResCode.NOT_FOUND).json({ error: 'Address not found' });
        }
        ...
        for (let item of orderedItems) {
            ...
            if (!product) {
                return res.status(HttpResCode.NOT_FOUND).json({ error: `Product with ID ${item.productId} not found` });
            }
            if (item.quantity > product.quantity) {
                return res.status(HttpResCode.BAD_REQUEST).json({ error: `Insufficient stock for product: ${product.productName}` });
            }
        }
...
export const handlePaymentFailure = async (req, res) => {
    try {
        ...
        if (!orderId) {
            return res.status(HttpResCode.BAD_REQUEST).json({
                success: false,
        ...
        const order = await Order.findById(orderId);
        if (!order) {
            return res.status(HttpResCode.NOT_FOUND).json({
                success: false,
