...
export const pagenotfound = async (req,res) => {
    try {
        ...
        res.status(HttpResCode.NOT_FOUND).send("Page Not Found")
    }
}
...
export const signup = async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            return res.status(HttpResCode.BAD_REQUEST).json({ error: 'User with this email already exists' });
        }
        ...
        if (referralCode) {
            ...
            if (!referrer) {
                return res.status(HttpResCode.BAD_REQUEST).json({ error: 'Invalid referral code' });
            }
...
export const resendOtp = async (req, res) => {
    try {
        ...
        if (!req.session.userData || !req.session.userData.email) {
            return res.status(HttpResCode.BAD_REQUEST).json({ success: false, message: "Email not found in session" });
        }
...
export const verifyotp = async (req, res) => {
    try {
        ...
        if (otp === req.session.userOtp) {
            ...
            res.status(HttpResCode.BAD_REQUEST).json({ success: false, message: 'Invalid OTP, please try again' });
...
export const login = async (req, res) => {
    try {
        ...
        if (!findUser) {
            return res.status(HttpResCode.BAD_REQUEST).json({ message: "User not found, please register" });
        }

        if (findUser.isBlocked) {
            return res.status(HttpResCode.BAD_REQUEST).json({ message: "User is blocked by Admin" });
        }

        const passwordMatch = await bcrypt.compare(password, findUser.password);
        if (!passwordMatch) {
            return res.status(HttpResCode.BAD_REQUEST).json({ message: "Incorrect password" });
        }
...
export const deleteReview = async (req, res) => {
    ...
    try {
        ...
        if (reviewIndex === -1) {
            return res.status(HttpResCode.NOT_FOUND).send('Review not found or you do not have permission to delete this review');
        }
...
export const resetPasswords = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(HttpResCode.NOT_FOUND).json({ success: false, message: 'User not found' });
        }
